...
class Recommender:
    ...
    def suggest_product_for(self, products, max_results=6):
        ...
        if len(products)==1:
            ...
            # несколько товаров, объеденить баллы всех товаров, сохранить полученное сортированное множество во временном ключе
            keys = [self.get_product_key(id) for id in product_ids]
            r.zunionstore(tmp_key, keys)
            # удалить идентификаторы товаров, для которых дается рекомендация
            r.zrem(tmp_key, *product_ids)
            # получить идентификаторы товаров по их количеству, сортировка по убывантю
